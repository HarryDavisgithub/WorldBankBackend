...
app.use(allowCors())
	...
	.get("/compare/:firstCountry/:secondCountry/:indicator/info", getTwoCountriesIndicatorInfo)
	.get(
...
async function getCountryInfo(server) {
    ...
    WHERE CountryName = $1
  `;
	const allCountryInfo = (await db.queryObject({ text: query, args: [formattedCountry] })).rows;
...
async function getCountryIndicatorYearInfo(server) {
  ...
  `;
	...
	if (checkReturnedArrayLength(allCountryIndicatorYearInfo)) {
		const countryIndicatorYearInfo = allCountryIndicatorYearInfo.map((country) => {
			return {
...
async function getTwoCountriesIndicatorInfo(server) {
  ...
  `;
	...
	const twoCountriesIndicatorInfo = allTwoCountriesIndicatorInfo.map((country) => {
		return {
			Country: country.countryname,
			Indicator: country.indicatorname,
			Year: country.year,
...
async function getTwoCountriesIndicatorYearInfo(server) {
  ...
  `;
	const allTwoCountriesIndicatorYearInfo = (
		await db.queryObject({
			text: query,
			args: [formattedFirstCountry, formattedSecondCountry, formattedIndicator, year],
		})
	).rows;
	const twoCountriesIndicatorYearInfo = allTwoCountriesIndicatorYearInfo.map((country) => {
		return {
			Country: country.countryname,
			Indicator: country.indicatorname,
			Year: country.year,
